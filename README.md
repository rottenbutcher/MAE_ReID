...
```bash
# Adjust --data-root to point at your Real_Ship dataset
python tools/create_classification_splits.py --data-root ./data/Ship_Real_ext
```

> **Note:** After generating the splits, update the `cls_dim` value inside
> `cfgs/finetune_marine_classification.yaml` so it matches the total number of
> classes discovered by the script (e.g., 22).
