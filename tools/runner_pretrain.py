...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data_tuple) in enumerate(train_dataloader):
            num_iter += 1
            ...
            if isinstance(loss_tuple, (list, tuple)):
                # Each GPU returns a loss tensor of different size (due to viewpoint mask)
                # We must .mean() each one *before* stacking them
                scalar_losses = [l.mean() for l in loss_tuple]
                loss = torch.stack(scalar_losses).mean()
            else:
                # Single GPU or model returns a single tensor
                loss = loss_tuple.mean() 
